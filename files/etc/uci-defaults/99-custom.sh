...
if [ "$count" -eq 1 ]; then
    ...
    uci commit network
    echo "Single NIC device: set static IP 192.168.50.5 with GW 192.168.50.1 and DNS at $(date)" >>$LOGFILE
    ...
    uci commit dhcp
    echo "DHCP server enabled on LAN interface (range: 192.168.50.100 ~ .249) at $(date)" >>$LOGFILE
